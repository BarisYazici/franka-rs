...
```
franka-rerun csv <log.csv> --robot fr3|fer [--urdf PATH] [-o out.rrd] [--every N] [--meshes DIR | --no-meshes] [--layout default|demo] [--budget V,A,J]
```
...
- `-o` defaults to the input path with `.rrd`; `--every N` logs every N-th row to the 3D
  scene (the time series always carry every row); the arm is drawn with Franka's built-in
  link meshes, `--meshes DIR` with other ones and `--no-meshes` with none (below);
  `--layout demo` swaps the viewer layout for the screen-capture one
...
```
franka-rerun log <records.json> --robot fr3|fer [--urdf PATH] [-o out.rrd] [--every N] [--meshes DIR | --no-meshes] [--force-scale M] [--noise-floor NM]
```

`--robot`, `--urdf`, `-o`, `--every`, `--meshes` and `--no-meshes` as above; `--force-scale` is the length
...
the record index in milliseconds when `time` never changes over the log. A record taken live
carries the host's `CLOCK_MONOTONIC` as well and is logged on `host_time` too (see *Record
live*), which is the axis on which two robots -- two controllers, so two unrelated
`robot_time`s -- and a camera node line up. `FlightOptions::prefix` puts every path below under
a name (`L/joints/q`), which is what lets several robots share one recording; it is empty by
default.
...
and the contact link as tabs, `F_ext`, the position (with the orientation, the per-axis plots
and the derivatives of a Cartesian command as tabs behind it), `dq`, and the four flag plots as
tabs; the events along the bottom. `flight::send_blueprint` takes a `Layout`, the prefix of
every robot the recording holds and the timeline the time panel opens on, and repeats those
views per robot: a blueprint sent at all turns the viewer's automatic layout off, so an arm no
view names would be in the file and not on screen. The tool prints a `Summary`: records and span, rising edges per
...
joint 4's axis with links 3 and 4 tied. For a push on the forearm the estimated force points
along the robot's own `O_F_ext_hat_K` and is larger than it, because the robot's wrench is the
end-effector equivalent of a force acting closer to the base.

### Meshes

The arm is drawn with Franka's link meshes on top of the skeleton, built in by default: the
`builtin-meshes` feature (on by default) brings `franka-description`, franka_description's
FR3 and FER visual meshes (Apache-2.0, Franka Robotics' work; commit `7aeeddc`) decimated by
`tools/franka-meshes` to about 15 % of their triangles (30 % for the hand, the finger whole),
at 0.2 to 1.0 mm of surface deviation at the 99th percentile per mesh.
That is about 0.7 MB of static mesh per arm in a recording; the command line prints
`meshes: 8 link mesh(es), hand, fingers from builtin fr3 (15 %, franka_description
7aeeddc)`. `--meshes DIR` draws other files instead: `DIR/link0 .. link7`, `hand` and
`finger`, as `.glb` (or `.gltf`, `.obj`, `.stl`, `.dae` -- Rerun 0.37 reads Collada too),
the full-resolution ones for example, which `tools/franka-meshes/convert.py` makes without
`--decimate`; `--no-meshes` draws the skeleton alone. In the library that is
`MeshChoice::{Builtin, Dir, Off}` in `FlightOptions::meshes` (and so
`RecorderOptions::flight.meshes`), and `MeshChoice::resolve` for the `meshes` argument of
`CommanderLog::record`; a build that must not carry the meshes takes the crate with
`default-features = false`. Each mesh is logged once as an `Asset3D` at
`world/links/<name>/mesh`, and every record puts a `Transform3D` on `world/links/<name>`.
...
checks on the built-in meshes, and on converted files when `FRANKA_MESHES=DIR` is set. The hand is mounted as the
...
is in the directory; leave it out in that case (or pass `--no-meshes`).
...
`log`; set `HOST_TIMELINE` too, or the rows are invisible while the viewer is on it).

`push` also reads the host's `CLOCK_MONOTONIC` (`franka::realtime::monotonic_ns`, a vDSO read,
no allocation) and every row it writes carries that as `host_time` beside `robot_time`. The
stamp is taken where the record is, on the control thread, not where it is written; an offline
replay has no host clock for a record and writes `robot_time` alone.
...
`automatic_error_recovery()`, and finishes the recorder.
...
FRANKA_REALTIME=enforce cargo run --release -p franka-rerun --example commander_live -- \
    <robot-hostname> --live 127.0.0.1:9876 [--meshes DIR] [--bridged | --raw] \
...
with the anchoring, the deviation guard and the settle logic inside the crate's loop
thread; there is no CSV `--log`, the recording carries all of it. The loop's observer does
...
On franka-sim (`--enforce-motion-limits`, FR3 image, `FRANKA_REALTIME=ignore`) bridged runs
the sequence to the end with no records dropped and the peak sent speed under the budget; raw
is refused at the first step with `cartesian_motion_generator_velocity_discontinuity` (the
sent position's first difference is 50 m/s) and recovers. The loop's rate-limiter backstop
references the twist it sent,
...
streams in and catch up afterwards; the `--out` file has everything regardless.
...
`log_static`); `franka_rerun::meshes::Meshes` finds (or takes the built-in) and logs the link
meshes, `MeshChoice` picks them;
