...
use franka::Model;
use franka_rerun::{commander, flight, CommanderLog, FlightOptions, RobotKind};
...
const USAGE: &str = "Usage: franka-rerun csv <log.csv> --robot fr3|fer [--urdf PATH] [-o out.rrd] \
                     [--every N]\n       franka-rerun log <records.json> --robot fr3|fer \
                     [--urdf PATH] [-o out.rrd] [--every N] [--force-scale M]\n\n  csv           \
                     replay a nonrealtime_commander --log CSV\n  log           replay a control \
                     log saved with franka_rerun::save_records (JSON)\n  --robot fr3   FR3 \
                     limits, model from --urdf (default: the repository's tests/data/fr3.urdf)\n  \
                     --robot fer   FER limits, the crate's built-in FER model\n  -o PATH       \
                     output recording (default: the input with .rrd)\n  --every N     log every \
                     N-th row to the 3D scene (default 1)\n  --force-scale M  log only: metres \
                     of arrow per newton of external force (default 0.01)";
...
fn parse_args(args: &[String]) -> Result<Args, String> {
    ...
    let (mut every, mut force_scale) = (1, FlightOptions::default().force_scale);
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--force-scale" => {
                let text = value(&mut i, "--force-scale")?;
                force_scale = text
                    .parse::<f64>()
                    .ok()
                    .filter(|&s| s > 0.0 && s.is_finite())
                    .ok_or_else(|| format!("--force-scale {text:?}: want a positive number"))?;
            }
...
fn replay_csv(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let summary = log.record(&rec, &model, &limits, args.every)?;
