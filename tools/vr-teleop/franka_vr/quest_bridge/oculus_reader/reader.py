# Vendored from github.com/rail-berkeley/oculus_reader at commit 1168953a
# (Apache-2.0; see the LICENSE beside this file and ../../../NOTICE). THREE
# edits, and only these three:
#   1. package-relative imports, because this lives at
#      franka_vr.quest_bridge.oculus_reader rather than at the import root;
#   2. the Wi-Fi retry in get_network_device() calls get_network_device(), as
#      upstream main does -- the pinned commit calls get_device(), which takes
#      no arguments, so that path raised TypeError instead of retrying;
#   3. install()'s default APK is fetch_apk.fetch(): the pinned build in the
#      user's data dir, downloaded if missing and hash-checked every time, not
#      a file inside the installed package. __init__ installs through it too.
# Everything else -- the logcat tag, the ppadb client, the adb/USB/Wi-Fi device
# discovery, process_data's 4x4 parser -- is untouched on purpose: it is
# known-good and hardware-verified. Do not "improve" it.
from .FPS_counter import FPSCounter
from .buttons_parser import parse_buttons
from .fetch_apk import fetch
import numpy as np
import threading
import time
import os
