...
//
// Minimal SHA-256 implementation (FIPS 180-4) used only to stamp the URDF hash
// into the generated fixture header. Not performance critical.
#pragma once
...
class Sha256 {
 public:
  ...
  std::string hexdigest() {
    ...
    for (int j = 0; j < 8; ++j) {
      buffer_[63 - static_cast<size_t>(j)] = static_cast<uint8_t>(bit_len_ >> (8 * j));
    }
    transform(buffer_.data());

    for (int j = 0; j < 4; ++j) {
      for (int k = 0; k < 8; ++k) {
        hash[static_cast<size_t>(k * 4 + j)] =
            static_cast<uint8_t>((state_[static_cast<size_t>(k)] >> (24 - j * 8)) & 0xFF);
      }
    }
