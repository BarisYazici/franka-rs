...
| `O_T_EE` is the joint-7 frame | both | 0.107 m short of the flange along the tool z axis (measured 0.10700011 m), with `F_T_EE` and `NE_T_EE` both identity. franka-node's lead check measures against `O_T_EE`, so on the simulator every target leads by the offset: its tests run with `max_lead` off or raised by 0.107 m. | `sim_commands.rs`, `sim_v5_commands.rs`, `franka-node/tests/sim_node.rs` |
