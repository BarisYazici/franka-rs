...
- It does not run under `PREEMPT_RT`, and neither does CI, so every test and example connects
  with `RealtimeConfig::Ignore`. Timing measured against it is a same-host A/B, not an FCI
...
| `O_T_EE` is the joint-7 frame | both | 0.107 m short of the flange along the tool z axis, with `F_T_EE` and `NE_T_EE` both identity. franka-node's lead check measures against `O_T_EE`, so on the simulator every target leads by the offset: its tests run with `max_lead` off or raised by 0.107 m. | `sim_commands.rs`, `sim_v5_commands.rs`, `franka-node/tests/sim_node.rs` |
...
| joint 1 ignores velocity commands | `panda-v5` | The velocity servo uses FR3 gains; on the Menagerie `franka_emika_panda` model joint 1 oscillates at the 500 Hz Nyquist frequency, so the clipped torque averages to zero and velocity commands produce little net motion. Position commands on joint 1 work. | `sim_v5_motions.rs` (`active_control` module) |
...
10 rad/s², is the FR3's published limit at scale 1.0. The `panda-v5` image has
