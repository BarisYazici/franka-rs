...
| `joints/q_goal`, `joints/dq_goal`, `joints/cap_scale`, `joints/tau_envelope` | a live recording of target control's torque backend (`Recorder::push_torque_at`, which franka-node uses): the joint goal, its velocity, the fraction of the generator's step the goal carried when a joint limit cut it (exactly 1 when none did), the velocity envelope's torque: the barrier's, less the law's torque faded out along a joint's motion above the fade's start |
| `joints/pinned`, `joints/tau_position`, `ik/stall`, `ik/passes` | the same recording's joint position limit guard: per joint the bound the goal was held on (0 free, ∓1 lower / upper position, ∓2 velocity), the position envelope's torque (the spring less the law's torque faded toward a limit), the stall pressure and flag, the IK's active-set passes (see [the guard](../reference/impedance.md#the-joint-position-limit-guard)) |
| `ik/step`, `ik/blend`, `ik/held`, `ik/error` | the same recording's solve: the largest raw step norm of the cycle's solves and how much of it the box clipped off (rad), the position-first blend, whether the goal was held (the IK stalled, or a wall standing) and how long since each of the two blocks was last pushed on a wall (−1 for none), and the weighted residual (see [a self-motion the law cannot see](../reference/impedance.md#near-full-stretch-a-self-motion-the-law-cannot-see)) |
...
| `ee/orientation/commanded` | the rotation of the pose the command carried, as a quaternion `xyzw`: the only orientation a torque-backend recording has, the robot's own `O_T_EE_c` echo being zero there |
| `ee/velocity`, `ee/leash` | the generator's end-of-cycle twist (m/s, rad/s) and what the [leash](../reference/impedance.md#anchoring-without-an-echo-the-leash) took off the anchor (m, rad) |
